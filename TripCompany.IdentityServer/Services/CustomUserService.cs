/* The customuserservice class: provides semantics for users to authenticated local accounts, as well as external accounts. It also provides identity and claims to identity server needed for tokens and the user info endpoint. Additionally, the user service can control the workflow a user will experience at login time, before being allowed to return to the client application. This is useful for things such as additional login requirements, such as to factor authentication or other custom requirements, such as accepting an end user license agreement. In short, if we want to customize anything having to do with authentication, we'll want to use a custom user service. 
  UserServiceBase class: 
  */
...
namespace TripCompany.IdentityServer.Services
{
    public class CustomUserService : UserServiceBase
    {
        ...
        /* Authenticatelocalasync() method is called for local authentication, ie whenever the user uses the username and password dialog. It's in this method that we can check whether or not the passed in username and password exist and match.*/
        ...
        /*Getprofiledataasync() method is called whenever claims about the user are requested. For example, during token creation or via the user info endpoint. So it's here we have to get the claims for the user.*/
        ...
        /* Isactiveasync() method is called whenever identity server needs to determine if the user is still considered valid or active, for example, if the users account has been deactivated since they logged in.*/
        ...
        /*authenticateexternalasync method is called when the user uses an external identity provider to authenticate. It allows the user service to map an external user to a local user.*/
        ...
        /* Postauthenticateasync() is called after the user has successfully authenticated, but before they are returned to the client application. It allows a consolidated place to check for custom user workflows after all of the other authentication methods.*/
        ...
        /* Pre-authenticateasync() is called before the log in page is shown. This allows the user service to determine if the user is already authenticated by some out of bound mechanism, like client certificates. And it can prevent the login page from being shown. 
         * Signoutasync(), which is called whenever the user signs out. We'll encounter these throughout the module.*/
